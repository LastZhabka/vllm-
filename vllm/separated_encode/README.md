...
Update provides a toy proxy implementation and scripts to startup the EPD Disaggregated vLLM. There are two scripts, one of them is used to run EPD Disaggregated vLLM on 1 GPUs, and another is used to run it on 2 GPUs. Any number of GPUs is supported by components, but you need to implement proxy to route the requests.
